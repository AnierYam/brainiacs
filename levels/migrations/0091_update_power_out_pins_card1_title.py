...
def update_power_out_pins_card1_title(apps, schema_editor):
    ...
    LessonCard.objects.filter(
        lesson=lesson,
        order=1,
    ).update(title="What are the Power Out Pins")


class Migration(migrations.Migration):
    dependencies = [
        ("levels", "0090_update_power_out_pins_card1_image_v3"),
    ]

    operations = [
        migrations.RunPython(update_power_out_pins_card1_title),
    ]
